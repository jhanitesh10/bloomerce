...
export default function ImageExportSlideOver({ onClose, skus = [], filtered = [], selected = [], references = {}, isEmbedded = false }) {
  const [scope, setScope] = useState(() => {
    ...
    const selCount = selected instanceof Set ? selected.size : (selected?.length || 0);
    if (saved === 'selected' && selCount === 0) return 'filtered';
    return saved || (selCount > 0 ? 'selected' : 'filtered');
  });
  ...
  const targetData = useMemo(() => {
    const selectedArr = selected instanceof Set ? Array.from(selected).map(id => skus.find(s => s.id === id)).filter(Boolean) : (selected || []);
    if (scope === 'selected') return selectedArr;
  ...
  const content = (
    <div className={cn("flex flex-col h-full", !isEmbedded && "fixed inset-y-0 right-0 z-[100] w-full md:max-w-xl bg-[var(--color-background)] border-l border-[var(--color-border)] shadow-2xl animate-in slide-in-from-right duration-300")}>
      
      <div className="flex items-center justify-between px-4 sm:px-5 py-3 sm:py-3.5 border-b border-[var(--color-border)] shrink-0 bg-[var(--color-card)] w-full">
        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
          ...
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-[var(--color-primary)]/10 flex items-center justify-center text-[var(--color-primary)] shadow-inner shrink-0">
               <ImageIcon size={18} className="sm:size-5" />
            </div>
            <div className="min-w-0 overflow-hidden">
              <h2 className="text-sm sm:text-base font-bold text-[var(--color-foreground)] leading-tight truncate">Export Assets</h2>
              <p className="text-[9px] sm:text-[10px] text-[var(--color-muted-foreground)] font-bold uppercase tracking-wider truncate">Structure Designer</p>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          >
            {isExporting ? <Loader2 size={14} className="animate-spin sm:size-4" /> : <Download size={14} className="sm:size-4" />}
            <span className="hidden sm:inline">Download {targetData.length} SKUs Assets</span>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-8 no-scrollbar bg-[var(--color-background)]">
        <section className="space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
             {[
              { id: 'selected', label: 'Current Page', count: selCount, desc: 'Selected SKUs' },
          ...
          <div className="bg-[var(--color-muted)]/20 rounded-3xl p-6 border border-[var(--color-border)] space-y-8 relative overflow-hidden">
            <div className="space-y-4">
              ...
              <div className="space-y-1 relative">
                {!flattenHierarchy && <div className="absolute left-4 top-4 bottom-4 w-px bg-[var(--color-border)]" />}
                {!flattenHierarchy ? (
                  folderLevels.map((tagId, idx) => (
                    <div key={idx} className="flex items-center gap-3 animate-in fade-in slide-in-from-left-2 duration-200 relative group" style={{ marginLeft: `${idx * 20}px` }}>
                       {idx > 0 && <div className="absolute -left-5 top-1/2 -translate-y-1/2 w-5 h-px bg-[var(--color-border)]" />}
                       <div className={cn("w-10 h-10 rounded-xl border flex items-center justify-center shrink-0 transition-all", tagId === 'none' ? "bg-[var(--color-card)] border-dashed border-[var(--color-border)] text-[var(--color-muted-foreground)]" : "bg-[var(--color-card)] border-[var(--color-border)] text-[var(--color-muted-foreground)] shadow-sm")}>
                          <Folder size={16} />
                       </div>
                       <div className="flex-1 relative">
                          <select value={tagId} onChange={(e) => updateFolderLevel(idx, e.target.value)} className={cn("w-full h-10 pl-3 pr-8 bg-[var(--color-card)] border rounded-xl text-xs font-bold transition-all outline-none appearance-none cursor-pointer", tagId === 'none' ? "border-dashed border-[var(--color-border)] text-[var(--color-muted-foreground)] italic" : "border-[var(--color-border)] text-[var(--color-foreground)] focus:ring-2 focus:ring-[var(--color-primary)]/20 focus:border-[var(--color-primary)]")}>
                       ...
                       {folderLevels.length > 1 && (
                         <button onClick={() => removeFolderLevel(idx)} className="p-2.5 text-[var(--color-muted-foreground)] hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100"><Trash2 size={16} /></button>
                       )}
                    </div>
                  ))
                ) : (
                  <div className="p-5 bg-[var(--color-card)] border-dashed border-2 border-[var(--color-border)] rounded-3xl flex items-center justify-center gap-3 text-[var(--color-muted-foreground)] italic">
                     <Folder size={16} /><span className="text-xs font-medium">Automatic SKU folder mode enabled</span>
                  </div>
                )}
              </div>
            </div>
            <div className="space-y-4 pt-4 border-t border-[var(--color-border)]">
              <div className="flex items-center justify-between px-1">
               ...
               <div className="flex flex-wrap items-center gap-2 p-3 bg-[var(--color-muted)]/40 border border-[var(--color-border)] rounded-2xl min-h-[64px]">
                  <div className="w-10 h-10 rounded-xl bg-[var(--color-card)] border border-[var(--color-border)] flex items-center justify-center text-[var(--color-primary)] shadow-sm shrink-0"><File size={16} /></div>
                    ...
                    <React.Fragment key={idx}>
                       ...
                       <div className="flex items-center gap-1 p-1 bg-[var(--color-card)] border border-[var(--color-border)] rounded-2xl shadow-sm group">
                          <div className="relative">
                             <select value={tagId} onChange={(e) => updateFileSegment(idx, e.target.value)} className={cn("h-8 pl-3 pr-7 bg-[var(--color-primary)]/5 border-none rounded-xl text-[11px] font-black text-[var(--color-primary)] appearance-none outline-none cursor-pointer min-w-[100px]", tagId === 'none' && "text-[var(--color-muted-foreground)] italic bg-[var(--color-muted)] font-normal")}>
                          ...
                          {fileSegments.length > 1 && (
                            <button onClick={() => removeFileSegment(idx)} className="p-1 px-2 text-[var(--color-muted-foreground)] hover:text-red-500 transition-colors"><Trash2 size={14} /></button>
                          )}
                       </div>
                    </React.Fragment>
                  ))}
            ...
            <div className="flex flex-wrap items-center gap-1 text-[11px] font-mono leading-relaxed truncate">
              ...
              {previewPath.split('/').map((seg, i, arr) => (
                <React.Fragment key={i}>
                  <span className={cn("transition-all duration-300", i === arr.length - 1 ? "text-[var(--color-primary)] font-bold bg-[var(--color-primary)]/10 px-1 rounded-sm" : "text-[var(--color-muted-foreground)]")}>{seg || '?'}</span>
